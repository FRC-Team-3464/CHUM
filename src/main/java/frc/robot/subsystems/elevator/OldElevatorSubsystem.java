...
public class OldElevatorSubsystem extends SubsystemBase {
    ...
    private final ProfiledPIDController elevatorController = new ProfiledPIDController(0.8, 0, 0,
            new TrapezoidProfile.Constraints(140, 800));
    private final ElevatorFeedforward elevatorFeedforward = new ElevatorFeedforward(.081, .254, 1, .01); // to be used
                                                                                                         // in the
                                                                                                         // future
                                                                                                         // hopefully
    ...
    public OldElevatorSubsystem() {
        ...
        // sets the right elevator motor to actually be configured to the code that sets
        // the configuration
    ...
    private static OldElevatorSubsystem instance = null;

    public static OldElevatorSubsystem getInstance() {
        if (instance == null) {
            instance = new OldElevatorSubsystem();
        }
        return instance;
    }

    public void setElevateTarget(double target) {
        // checking which direction the elevator has to travel in to reach the target
        // parameter after accounting for limit switches
        System.out.println("setElevateTarget should be running");
        System.out
                .println(Math.abs(Constants.ArmevatorConstants.kElevatorGearRatio * (target) - getElevatorPosition()));
    ...
    // setting the default speed of the left elevator at zero while also accounting
    // for limit switches
    ...
    public void periodic() {
        ...
        SmartDashboard.putNumber("Elevator Voltage",
                leftElevator.getAppliedOutput() * RobotController.getBatteryVoltage());
